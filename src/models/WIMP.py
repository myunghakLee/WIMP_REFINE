...
class WIMP(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--Ndist", type=float, default = 3.0, help="between K-heads distant")
    ...
    def __init__(self, hparams, hidden_dim=128):
        ...
        self.repeat = lambda tensor, num_reps, k = 6 : torch.cat([a.unsqueeze(1).repeat(1,k).flatten() for a in tensor], axis=0)#.reshape(len(tensor), num_reps**2)
    ...
    def eval_preds(self, preds, target_dict, waypoint_preds=None, k = 6, min_dist_threshold = 6):
